//
//  QAQuestionCreateViewController.swift
//  homework
//
...
class QAQuestionCreateViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func initiateConfirmButton() {
        let confirmButton = UIBarButtonItem(title: "发布问题", style: .Plain, target: self, action: #selector(self.confirmButtonOnClick))
    ...
    func confirmButtonOnClick(sender: AnyObject) {
        ...
        guard let content = self.questionContent else {
            AlertHelper(viewController: self).showPromptAlertView("问题不能为空")
            return
        }
        APIQAQuestionCreate(vc: self).run(classroomUUID, anonymous: self.anonymous, content: content)
    }
